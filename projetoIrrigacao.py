...
class IrrigationApp:
    def __init__(self, root):
        ...
        self.time_label = tk.Label(root, text="Hora Programada (HH:MM): ")
        self.time_label.pack()

        self.hour_spinbox = tk.Spinbox(root, from_=0, to=23, width=5)
        self.hour_spinbox.pack()

        self.minute_spinbox = tk.Spinbox(root, from_=0, to=59, width=5)
    ...
    def irrigate(duration, ip):
        try:
            response = requests.get(f"http://{ip}/on")
            if response.status_code == 200:
                messagebox.showinfo("Info", "Irrigação Ligada")
            time.sleep(duration)
    ...
    def auto_irrigation(self):
        while True:
            ...
            scheduled_minute = int(self.minute_spinbox.get())
            if current_time.tm_hour == scheduled_hour == scheduled_hour and current_time.tm_min == scheduled_minute:
                duration = int(self.duration_entry.get()) * 60  # Converte minutos para segundos
                ip = self.ip_entry.get()
